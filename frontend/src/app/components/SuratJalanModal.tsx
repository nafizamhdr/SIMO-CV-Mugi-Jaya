      ...
      >
        <div className="flex items-center justify-between border-b border-gray-200 pb-4 mb-4">
          <div>
            ...
            >
              {s.suratJalan || "SJ-2026-" + s.id.slice(-4)}
            </div>
          </div>
        ...
        <table className="w-full text-sm mb-4">
          <tbody>
            {[
              ["No. Pengiriman", s.id, true],
              ["QC Certificate", s.cert || "-", true],
              ["Vendor Ekspedisi", s.vendor, false],
              ["Driver", s.driver || "-", false],
              ["No. Kendaraan", s.plate || "-", true],
              ["No. Polis Asuransi", s.polis, true],
              ["Tujuan", s.tujuan, false],
            ].map(([label, val, mono]) => (
              <tr key={String(label)} className="border-t border-gray-100">
                <td className="py-2 text-gray-500 w-44">{label}</td>
                <td
                  className={`py-2 font-semibold ${label === "QC Certificate" ? "text-green-700" : "text-gray-900"}`}
                  style={
